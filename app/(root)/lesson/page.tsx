...
const LessonPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (activities.notes) {
        promises.push(generateNotes(formData));
      }
      ...
      for (const result of results) {
        if (result.success) {
          ...
          if ("notes" in result) {
            setGeneratedNotes(result.notes || null);
          }
        } else {
  ...
  const handleDownloadNotes = async () => {
    if (!generatedNotes) {
      setError("No notes content available for download.");
      return;
    }
    ...
    try {
      ...
      const htmlContent = generatedNotes
        .replace(
          /^# (.+)$/gm,
          '<h1 style="color: #3f51b5; font-size: 28px; font-weight: bold; margin: 30px 0 15px 0; border-bottom: 3px solid #3f51b5; padding-bottom: 8px; text-align: center;">$1</h1>'
        )
      ...
      if (!canvas || canvas.width === 0 || canvas.height === 0) {
        throw new Error("Failed to capture notes content");
      }
      ...
      <div className="container mx-auto px-6 py-12 max-w-6xl">
        ...
        {generatedNotes && (
          <div className="mt-8 bg-card border border-border rounded-lg p-8">
            ...
            <div
              id="notes-content"
              className="notes-content prose prose-lg max-w-none prose-headings:text-primary prose-headings:font-bold prose-p:text-foreground prose-li:text-foreground prose-strong:text-primary prose-blockquote:border-l-primary prose-blockquote:bg-primary/5 prose-blockquote:pl-4 prose-blockquote:py-2 prose-blockquote:rounded-r"
            >
              <ReactMarkdown
                remarkPlugins={[remarkGfm]}
                components={{
